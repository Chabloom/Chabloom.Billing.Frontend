...
export class ApplicationUsersApi extends BaseApi<ApplicationUserViewModel> implements BaseApiType<ApplicationUserViewModel> {
    ...
    readItems(token: string): Promise<Array<ApplicationUserViewModel> | string> {
        return this._readItems(token, `${this.baseUrl}`);
    }

    readItem(token: string, itemId: string): Promise<ApplicationUserViewModel | string> {
        return this._readItem(token, `${this.baseUrl}/${itemId}`);
    }

    addItem(token: string, item: ApplicationUserViewModel): Promise<string | undefined> {
        return this._addItem(token, `${this.baseUrl}`, item);
    }

    editItem(token: string, item: ApplicationUserViewModel): Promise<string | undefined> {
